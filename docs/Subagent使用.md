...
<Note>
  如果您需要多个代理并行工作并相互通信，请改为参阅 [agent teams](/zh-CN/agent-teams)。Subagents 在单个会话中工作；agent teams 跨多个独立会话进行协调。
</Note>
...
Claude Code 包括 Claude 在适当时自动使用的内置 subagents。每个都继承父对话的权限，并具有额外的工具限制。
...
<Steps>
  ...
  <Step title="选择工具">
    对于只读审查者，取消选择除 **Read-only tools** 之外的所有内容。如果保持所有工具选中，subagent 将继承主对话可用的所有工具。
  </Step>
...
| `tools`           | No       | Subagent 可以使用的 [Tools](#available-tools)。如果省略，继承所有工具                                                                                                                          |
| `disallowedTools` | No       | 要拒绝的工具，从继承或指定的列表中删除                                                                                                                                                           |
| `model`           | No       | 要使用的 [Model](#choose-a-model)：`sonnet`、`opus`、`haiku` 或 `inherit`。默认为 `inherit`                                                                                               |
...
| `skills`          | No       | 在启动时加载到 subagent 上下文中的 [Skills](/zh-CN/skills)。注入完整的技能内容，而不仅仅是可用于调用。Subagents 不继承父对话中的技能                                                                                      |
| `mcpServers`      | No       | 此 subagent 可用的 [MCP servers](/zh-CN/mcp)。每个条目要么是引用已配置服务器的服务器名称（例如 `"slack"`），要么是内联定义，其中服务器名称为键，完整的 [MCP server config](/zh-CN/mcp#configure-mcp-servers) 为值                   |
| `hooks`           | No       | 限定于此 subagent 的 [Lifecycle hooks](#define-hooks-for-subagents)                                                                                                                |
| `memory`          | No       | [Persistent memory scope](#enable-persistent-memory)：`user`、`project` 或 `local`。启用跨会话学习                                                                                       |
| `background`      | No       | 设置为 `true` 以始终将此 subagent 作为 [background task](#run-subagents-in-foreground-or-background) 运行。默认值：`false`                                                                     |
...
Subagents 可以使用 Claude Code 的任何 [internal tools](/zh-CN/settings#tools-available-to-claude)。默认情况下，subagents 继承主对话的所有工具，包括 MCP 工具。
...
当代理作为主线程运行 `claude --agent` 时，它可以使用 Agent 工具生成 subagents。要限制它可以生成的 subagent 类型，请在 `tools` 字段中使用 `Agent(agent_type)` 语法。
...
如果 `Agent` 完全从 `tools` 列表中省略，代理无法生成任何 subagents。此限制仅适用于作为主线程运行 `claude --agent` 的代理。Subagents 无法生成其他 subagents，因此 `Agent(agent_type)` 在 subagent 定义中无效。
...
| 工具 / 操作 | `default` | `acceptEdits` | `plan` | `dontAsk` | `bypassPermissions` |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Read | 自动通过 | 自动通过 | 自动通过 | 自动通过 | 自动通过 |
| Glob | 自动通过 | 自动通过 | 自动通过 | 自动通过 | 自动通过 |
| Grep | 自动通过 | 自动通过 | 自动通过 | 自动通过 | 自动通过 |
| Edit | 需确认 | 自动通过 | 禁止 | 自动拒绝 | 自动通过 |
| Write | 需确认 | 自动通过 | 禁止 | 自动拒绝 | 自动通过 |
| NotebookEdit | 需确认 | 自动通过 | 禁止 | 自动拒绝 | 自动通过 |
| Bash | 需确认 | 需确认 | 禁止 | 自动拒绝 | 自动通过 |
| WebFetch | 需确认 | 需确认 | 禁止 | 自动拒绝 | 自动通过 |
| WebSearch | 需确认 | 需确认 | 禁止 | 自动拒绝 | 自动通过 |
| Agent | 需确认 | 需确认 | 禁止 | 自动拒绝 | 自动通过 |

> **说明：**
> - **自动通过**：无需用户确认，直接执行
> - **需确认**：弹出权限提示，等待用户批准
> - **自动拒绝**：不弹提示，直接拒绝（但在 settings 中显式 allow 的工具仍可执行）
> - **禁止**：工具不可用，subagent 在只读模式下无法调用写入类工具
...
每个技能的完整内容被注入到 subagent 的上下文中，而不仅仅是可用于调用。Subagents 不继承父对话中的技能；您必须明确列出它们。
...
* Subagent 的系统提示还包括内存目录中 `MEMORY.md` 的前 200 行，以及在 `MEMORY.md` 超过 200 行时进行管理的说明。
...
您可以通过将特定 subagents 添加到 [settings](/zh-CN/settings#permission-settings) 中的 `deny` 数组来防止 Claude 使用它们。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 与 subagent 的 name 字段匹配。
...
支持所有 [hook events](/zh-CN/hooks#hook-events)。subagents 最常见的事件是：
...
Claude 根据您请求中的任务描述、subagent 配置中的 `description` 字段和当前上下文自动委托任务。要鼓励主动委托，请在 subagent 的 description 字段中包含"use proactively"之类的短语。
...
当您想要在主对话上下文中运行的可重用提示或工作流而不是隔离的 subagent 上下文时，请改为考虑 [Skills](/zh-CN/skills)。

<Note>
  Subagents 无法生成其他 subagents。如果您的工作流需要嵌套委托，请改用 [Skills](/zh-CN/skills) 或从主对话 [chain subagents](#chain-subagents)。
</Note>
...
* **会话持久化**：Subagent 转录在其会话中持久化。您可以通过恢复相同的会话在重启 Claude Code 后 [恢复 subagent](#resume-subagents)。
...
一个只读 subagent，在不修改代码的情况下审查代码。此示例展示了如何设计具有有限工具访问权限（无 Edit 或 Write）和详细提示的专注 subagent，该提示明确指定要查找的内容以及如何格式化输出。
